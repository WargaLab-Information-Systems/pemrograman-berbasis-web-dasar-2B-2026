<?php
    ...
    if (isset($_POST["simpan"])) {
        ...
        $query = mysqli_query($konek, "UPDATE tasks SET judul = '$judul', deskripsi = '$deskripsi', deadline = '$deadline', status = '$status' WHERE id = $id");
...
<body>
    <div class="w-full md:w-2/3 md:mx-auto flex justify-center items-center p-10 h-screen">
        <div class="bg-white w-full md:w-1/2 p-8 rounded-lg shadow-lg">
            <h1 class="text-center font-bold text-xl text-sky-500 mb-5">UPDATE TUGAS</h1>
            <form method="POST" action="">                    <label for="judul" class="block mb-2 text-slate-600 font-semibold">Nama Judul:</label>
